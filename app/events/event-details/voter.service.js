...
var VoterService = (function () {
    ...
    VoterService.prototype.deleteVoter = function (eventId, session, voterName) {
        ...
        this.http.delete("/api/events/" + eventId + "/sessions/\n            " + session.id + "/voters/" + voterName).catch(this.handleError).subscribe();
    };
    VoterService.prototype.addVoter = function (eventId, session, voterName) {
        session.voters.push(voterName);
        var headers = new http_1.Headers({ 'Content-Type': 'application/json' });
        var opsions = new http_1.RequestOptions({ headers: headers });
        var url = "/api/events/" + eventId + "/sessions/" + session.id + "/voters/" + voterName;
        this.http.post(url, JSON.stringify({}), opsions).catch(this.handleError).subscribe();
    };
